...
{
	public class OldGameEncryption : IClientEncryption
	{
		private String name;
		private uint key1;
		private uint key2;
		...
		public bool init(uint seed, byte[] buffer, int offset, int length)
		{
			...
			for (int i = 0; i < Configuration.LoginKeys.Length; ++i)
			{
				...
				Console.WriteLine("Trying {0}: {1}, {2}", Configuration.LoginKeys[i].Name, key1, key2);
				Console.WriteLine("packet: {0}", string.Join(string.Empty, Array.ConvertAll(packet, b => b.ToString("X2"))));
		...
		public OldGameEncryption(uint seed, uint key1, uint key2)
		{
